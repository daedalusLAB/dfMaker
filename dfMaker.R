...
#'
#' @param input.folder Path to the folder containing OpenPose JSON files.
#' @param config.path Path to the configuration file. If missing, default configuration is used.
#' @param output.file Name of the output file. If NULL, an auto-generated name is used.
#' @param output.path Path to save the output file. If NULL, saves in a default directory.
#' @param no_save Logical. If TRUE, the output is not saved to a file.
#' @param fast_scaling Logical. If TRUE, uses fast scaling for transformation.
#' @param transformation_coords Numeric vector of length 4 specifying transformation coordinates.
#' @return A data frame containing the processed keypoints data.
#' @export
dfMaker <- function(
    input.folder,
    config.path,
